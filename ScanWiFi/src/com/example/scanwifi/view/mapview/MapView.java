...
import com.example.scanwifi.utils.Tools;
...
public class MapView extends View {
	...
	public void run() {
	    while (true) {
		mIsRunning = true;
		PointF step = new PointF((mTargetPoint[0] - mMyLocationSymbol
			.getLocation().getX()) / 4,
			(mTargetPoint[1] - mMyLocationSymbol.getLocation()
				.getY()) / 4);
	...
	case MotionEvent.ACTION_MOVE:
		...
		if (event.getPointerCount() == 1) {
		    ...
		    calScreenRect();
		    Log.v("onTouch", "trans:" + delX + "," + delY + " matrix:"
			    + mMapMatrix.toShortString());
	...
	translateToSpecificPoint(location.getX(), location.getY());
    }
	...
	if (mMyLocationSymbol.getLocation() == null) {
		...
		mUpdateMyLocationAnimRunnable.setTarget(location.getX(),
			location.getY());
	    }
